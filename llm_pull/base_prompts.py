...
class ExampleSamplerMixin:
    ...
    def _handle_labels(
        ...
        label_mode = self._example_sampler_mixin_data["label_mode"]
    ...
    def handle_query_label(self, query: dict[str, str | torch.Tensor]) -> str:
    ...
    def sample_with_strategy(
        ...
        self._handle_labels(samples, dataset)
    ...
    def _similarity_sample(
        ...
        if "sentence_embeddings" not in self._example_sampler_mixin_data:
            self._example_sampler_mixin_data["sentence_embeddings"] = (
                ...
                ].encode([e["text"] for e in dataset], convert_to_tensor=True)
            )
        ...
        _, inds = torch.topk(cossims, k=shot)
...
class PromptBaseDataset(ExampleSamplerMixin, BaseDataset):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.query_prompt = (
            query_prompt
            or self.user_prompt
            or self.incontext_prompt[: self.incontext_prompt.find("{label}")]
        )
        ...
        # DEBUGGING MESSAGES
        self.log("Label set: " + ", ".join(self.label_set), "debug")
        example_prompt = self[0]
        self.log("Example prompt: " + example_prompt["text"], "debug")
    ...
    def _format_incontext_prompt(
        self, incontext_prompt_template: str, text: str, label: str
    ) -> str:
